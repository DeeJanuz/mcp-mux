...
Update installed plugins to their latest versions from the registry. Uses remote manifest resolution to discover available updates. If no plugin name is provided, updates all plugins with available updates. ZIP package updates replace plugin package files while preserving the plugin's local `preferences.json`, including setup answers and update policy.
...
Optional style preferences such as MCPViews Gronk Speak are plugin-provided setup-gated startup rules, not built-in MCPViews core questions. The plugin asks two independent questions: whether to enable `GronkSpeak` and whether to enable `PlainProse`. Each question supports Enable or Off. Choosing Off suppresses that rule's fresh install without changing the other choice.

`GronkSpeak` applies terse compression to nonpublic chat, status, findings, and ordinary technical reports. It yields to polished public, customer, regulated, or audience-sensitive writing. `PlainProse` applies clear complete prose across public and private writing without enabling GronkSpeak fragments. Each enabled rule includes its own adapted Unslop baseline: remove filler, promotional claims, chatbot phrasing, vague attribution, forced symmetry, and generic conclusions; prefer named sources, concrete facts, plain verbs, readable sentences, and grounded human voice. Both rules preserve exact code, commands, paths, identifiers, schemas, API names, errors, citations, values, and quotations.

After the user enables a choice, `startup_rule_actions` returns the matching agent-native rule. Manifest version changes update stale installed rule text through `auto_update` unless the project records `do_not_update`. Disabled and Off choices remain suppressed.
