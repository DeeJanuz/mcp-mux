...
fn builtin_renderer_definitions() -> Vec<RendererDef> {
    vec![
        RendererDef {
            ...
            rule: Some("Push to the companion window when the content benefits from visual diagrams. If it doesn't need a mermaid diagram, it doesn't need a push — just explain it in chat.\n\nEvery push MUST include at least one ```mermaid block. Use flowchart, sequenceDiagram, erDiagram, classDiagram, or graph TD/LR as appropriate. Content that warrants pushing: architecture, data flows, system diagrams, API designs, database schemas, networking topology, implementation plans with structural decisions.\n\nYour chat response should be a concise summary (context, next steps, decisions needed). The visual explanation with mermaid diagrams, tables, and formatted markdown goes to push_content.\n\nSUB-AGENT RESTRICTION: Only the main/coordinator agent may call push_content, push_review, and push_check. Sub-agents must NOT call these tools — they return results to the coordinator, which decides what to push.".into()),
        },
    ]
}
