...
Agents ask setup questions one at a time in returned order. MCPViews core questions are asked first, then installed plugin questions are asked sequentially; agents should explain the current question conversationally, use the question and option descriptions to help the user choose, show only the current question's options, and wait for the user's answer before moving on. Agents persist only the selected option's `persisted_rule`. Put long procedures in `prompt_definitions` and keep persisted setup rules short.
