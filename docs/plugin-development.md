...
Agents ask setup questions one at a time in returned order. Installed plugin questions are asked sequentially by group and by question order; agents should explain the current question conversationally, use the question and option descriptions to help the user choose, include `example_outputs` when present, show only the current question's options, and wait for the user's answer before moving on. Agents persist only the selected option's `persisted_rule`. Put long procedures in `prompt_definitions` and keep persisted setup rules short.
