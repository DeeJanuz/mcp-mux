...
| `setup_questions` | SetupQuestion[] | No | Optional setup-time questions returned by `mcpviews_setup`. Agents should ask exactly one question at a time in returned order, explain the current choice using the question and option descriptions, then persist only the selected option's compact `persisted_rule`, not every option or the full workflow. |
