...
| `mcp_prompts.rs` | MCP prompts protocol implementation. `ONBOARDING_PROMPT` constant with guided plugin discovery instructions. `builtin_prompt_definitions()` returns 4-tuples `(name, description, arguments, content)` pairing prompt content with definitions for extensibility (adding a new built-in prompt requires only adding a tuple entry). `list_prompts(state)` returns all available prompts (built-in + plugin, with plugin prompts namespaced as `{plugin}/{prompt}`). `get_prompt(name, arguments, state)` resolves a prompt by name (built-in via definition iteration, or plugin via `{plugin}/{prompt}` split) and returns MCP-formatted messages. `call_get_plugin_prompt(arguments, state)` fetches a plugin prompt file from disk with `{{arg}}` template substitution. Includes 1 unit test for `builtin_prompt_definitions` |
